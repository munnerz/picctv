...
LOGGER = getLogger(name="modules.Networking")

_connections = {}
_send_queue = Queue()
_process = Thread(target=self.run, args=(self._send_queue,))
...
def _create_connection(module_name):
    ...
    try:
        ...
        if _pickle_and_send(argument['node_name'], sock) and _pickle_and_send(module_name, sock):
